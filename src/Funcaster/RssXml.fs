...
let getDocument () = XDocument(XDeclaration("1.0", "UTF-8", null))
...
let tryGetRestrictions countries =
...
let getItem (item:Item) =
    ...
    |> Xml.append (
        ...
        |> Xml.attribute "length" (string item.Enclosure.Lenght)
    )
...
let getChannel (channel:Channel) =
    Xml.create "channel"
